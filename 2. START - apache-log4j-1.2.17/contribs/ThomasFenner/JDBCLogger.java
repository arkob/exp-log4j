...
public class JDBCLogger
{
	...
	*/
	...
	{
		if(!ready) if(!LogColumn.ready(this)) throw new Exception("JDBCLogger::appendSQL(), Not ready to append !");
...
class LogColumn
{
   ...
   boolean ignore = false;
	...
	Throws an exception, if an database-error occurs !
	*/
	public static void append(com.klopotek.utils.log.JDBCLogger jdbcLogger, String _msg) throws Exception
	{
		if(!jdbcLogger.ready) if(!jdbcLogger.ready()) throw new Exception("JDBCLogger::append(), Not ready to append !");

      if(jdbcLogger.sql != null)
      {
      	jdbcLogger.appendSQL(_msg);
         return;
      }

		LogColumn logcol;

		jdbcLogger.rs.moveToInsertRow();

		for(int i = 0; i< jdbcLogger.num; i++)
		{
        	logcol = (LogColumn) jdbcLogger.logcols.get(i);

			if(logcol.logtype == LogType.MSG)
			{
				jdbcLogger.rs.updateObject(logcol.name, _msg);
			}
			else if(logcol.logtype == LogType.ID)
			{
				jdbcLogger.rs.updateObject(logcol.name, logcol.idhandler.getID());
			}
			else if(logcol.logtype == LogType.STATIC)
			{
				jdbcLogger.rs.updateObject(logcol.name, logcol.value);
			}
			else if(logcol.logtype == LogType.TIMESTAMP)
			{
				jdbcLogger.rs.updateObject(logcol.name, new Timestamp((new java.util.Date()).getTime()));
			}
		}

		jdbcLogger.rs.insertRow();
	}
	...
	Throws an exception, if an database-error occurs !
	*/
	public static void configureTable(com.klopotek.utils.log.JDBCLogger jdbcLogger, String _table) throws Exception
	{
   	if(jdbcLogger.isconfigured) return;

		//Fill logcols with META-informations of the table-columns
		jdbcLogger.stmt = jdbcLogger.con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
		jdbcLogger.rs = jdbcLogger.stmt.executeQuery("SELECT * FROM " + _table + " WHERE 1 = 2");

		LogColumn logcol;

		ResultSetMetaData rsmd = jdbcLogger.rs.getMetaData();

		jdbcLogger.num = rsmd.getColumnCount();

		jdbcLogger.logcols = new ArrayList(jdbcLogger.num);

		for(int i = 1; i<= jdbcLogger.num; i++)
		{
         ...
         jdbcLogger.logcols.add(logcol);
		}

      jdbcLogger.table = _table;

		jdbcLogger.isconfigured = true;
	}
	 ...
	 * @param jdbcLogger
	 */
	public static boolean ready(com.klopotek.utils.log.JDBCLogger jdbcLogger)
	{
   	if(jdbcLogger.ready) return true;

		if(!jdbcLogger.isconfigured){ jdbcLogger.errormsg = "Not ready to append ! Call configure() first !"; return false;}

      //No need to doing the whole rest...
      if(jdbcLogger.sql != null)
      {
      	jdbcLogger.ready = true;
         return true;
      }
		...
		for(int i = 0; i< jdbcLogger.num; i++)
		{
         ...
         {
         	jdbcLogger.errormsg = "Not ready to append ! Column " + logcol.name + " is not nullable, and must be specified by setLogType() !";
            return false;
         }
			if(logcol.logtype == LogType.ID && logcol.idhandler == null)
         {
         	jdbcLogger.errormsg = "Not ready to append ! Column " + logcol.name + " is specified as an ID-column, and a JDBCIDHandler has to be set !";
            return false;
         }
			else if(logcol.logtype == LogType.STATIC && logcol.value == null)
         {
         	jdbcLogger.errormsg = "Not ready to append ! Column " + logcol.name + " is specified as a static field, and a value has to be set !";
            return false;
         }
      ...
      //create the column_list
		for(int i = 0; i< jdbcLogger.num; i++)
		{
         ...
         {
				if(jdbcLogger.column_list == null)
            {
            	jdbcLogger.column_list = logcol.name;
            }
            else jdbcLogger.column_list += ", " + logcol.name;
         }
		}

      try
      {
			jdbcLogger.rs = jdbcLogger.stmt.executeQuery("SELECT " + jdbcLogger.column_list + " FROM " + jdbcLogger.table + " WHERE 1 = 2");
		}
      catch(Exception e)
      {
			jdbcLogger.errormsg = "Not ready to append ! Cannot select columns '" + jdbcLogger.column_list + "' of table " + jdbcLogger.table + " !";
      	return false;
      }
   ...
   */
	public static void setLogType(boolean isconfigured, ArrayList logcols, int num, String sql, String _name, int _logtype, Object _value) throws Exception
	{
